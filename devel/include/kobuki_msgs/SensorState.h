  ...
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::kobuki_msgs::SensorState_<ContainerAllocator>& v)
  {
    ...
    for (size_t i = 0; i < v.bottom.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<uint16_t>::stream(s, true ? std::string() : indent + "    ", v.bottom[i]);
    }
    if (v.bottom.empty() || true)
      s << "]";
    if (true || !indent.empty())
    ...
    for (size_t i = 0; i < v.current.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<uint8_t>::stream(s, true ? std::string() : indent + "    ", v.current[i]);
    }
    if (v.current.empty() || true)
      s << "]";
    if (true || !indent.empty())
    ...
    for (size_t i = 0; i < v.analog_input.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<uint16_t>::stream(s, true ? std::string() : indent + "    ", v.analog_input[i]);
    }
    if (v.analog_input.empty() || true)
      s << "]";
  }
