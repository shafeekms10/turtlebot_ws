...
struct TrajectoryList_
{
  ...
  (void)_alloc;
   ...
   typedef std::vector< ::yocs_msgs::Trajectory_<ContainerAllocator> , typename std::allocator_traits<ContainerAllocator>::template rebind_alloc< ::yocs_msgs::Trajectory_<ContainerAllocator> >> _trajectories_type;
  ...
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::yocs_msgs::TrajectoryList_<ContainerAllocator>& v)
  {
    ...
    for (size_t i = 0; i < v.trajectories.size(); ++i)
    {
      if (false && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer< ::yocs_msgs::Trajectory_<ContainerAllocator> >::stream(s, false ? std::string() : indent + "    ", v.trajectories[i]);
    }
    if (v.trajectories.empty() || false)
      s << "]";
  }
