...
struct TableList_
{
  ...
  (void)_alloc;
   ...
   typedef std::vector< ::yocs_msgs::Table_<ContainerAllocator> , typename std::allocator_traits<ContainerAllocator>::template rebind_alloc< ::yocs_msgs::Table_<ContainerAllocator> >> _tables_type;
  _tables_type tables;
  ...
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::yocs_msgs::TableList_<ContainerAllocator>& v)
  {
    ...
    for (size_t i = 0; i < v.tables.size(); ++i)
    {
      if (false && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer< ::yocs_msgs::Table_<ContainerAllocator> >::stream(s, false ? std::string() : indent + "    ", v.tables[i]);
    }
    if (v.tables.empty() || false)
      s << "]";
  }
