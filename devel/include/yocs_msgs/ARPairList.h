...
struct ARPairList_
{
  ...
  (void)_alloc;
   ...
   typedef std::vector< ::yocs_msgs::ARPair_<ContainerAllocator> , typename std::allocator_traits<ContainerAllocator>::template rebind_alloc< ::yocs_msgs::ARPair_<ContainerAllocator> >> _pairs_type;
  _pairs_type pairs;
  ...
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::yocs_msgs::ARPairList_<ContainerAllocator>& v)
  {
    ...
    for (size_t i = 0; i < v.pairs.size(); ++i)
    {
      if (false && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer< ::yocs_msgs::ARPair_<ContainerAllocator> >::stream(s, false ? std::string() : indent + "    ", v.pairs[i]);
    }
    if (v.pairs.empty() || false)
      s << "]";
  }
