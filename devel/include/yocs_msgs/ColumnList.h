...
struct ColumnList_
{
  ...
  (void)_alloc;
   ...
   typedef std::vector< ::yocs_msgs::Column_<ContainerAllocator> , typename std::allocator_traits<ContainerAllocator>::template rebind_alloc< ::yocs_msgs::Column_<ContainerAllocator> >> _obstacles_type;
  ...
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::yocs_msgs::ColumnList_<ContainerAllocator>& v)
  {
    ...
    for (size_t i = 0; i < v.obstacles.size(); ++i)
    {
      if (false && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer< ::yocs_msgs::Column_<ContainerAllocator> >::stream(s, false ? std::string() : indent + "    ", v.obstacles[i]);
    }
    if (v.obstacles.empty() || false)
      s << "]";
  }
